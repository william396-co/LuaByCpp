...
public:
	// Lua gc operation
	struct GCObject* luaC_newobj( int tt_, size_t size);
	void luaC_step();
	void reallymarkobject(struct GCObject* gc);
...
private:
	...
	void propagatemark();
	lu_mem traversethread(lua_State* th);
	void propagateall();
	void atomic();
	void entersweep();
	void sweepstep();
	struct GCObject** sweeplist(struct GCObject** p, size_t count);
	lu_mem freeobj(struct GCObject* gco);
...
typedef struct global_State {
	...
	struct GCObject* graygain;
	lu_mem totalbytes;
	l_mem GCdebt;      // GCdebt will by negative
	lu_mem GCmemtrav;  // per gc step traverse memory bytes
	lu_mem GCestimate; // after finish a gc clcye, it records total memory bytes(totalbytes + GCdebt)
	int GCstepmul;
}global_State;
